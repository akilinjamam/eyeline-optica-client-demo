...
const RelatedService = ({data,serviceType, filterType}:{data:TFrame[], serviceType:string, filterType:string}, ) => {
        ...
        <div className='w-full mt-8 '>
            <div className="w-full sm:w-[75%] md:w-[85%] lg:w-[1200px] mx-auto 
                flex items-center justify-between px-4 md:block">
                    ...
                    {/* Title */}
                    <Title value={`RELATED ${serviceType}`} />
                    ...
                    <button onClick={() => router.push(`${filterType}`)}>
                        Shop Now
                    </button>
                    <ArrowRight />
                </div>
            ...
            <div className='hidden md:block lg:block'>
                <div className='flex items-center justify-around lg:w-[1200px] md:w-[90%] sm:w-[85%] mx-auto gap-5 relative'>
                    ...
                    <div ref={parentRef} className='w-[2600px] h-[300px] mx-auto flex items-center  gap-6 mt-2 overflow-x-hidden scroll-smooth  px-2'>
                        {
                            data?.slice(0,13)?.map(({color, name, brand, salesPrice, badge, images, otherImages, _id }: TFrame, index: number) => <GlassCard color={color} images={images} otherImages={otherImages}  badge={badge} salesPrice={salesPrice} name={name} brand={brand} key={index} _id={_id}/> )
                        }
                            </div>
            ...
            <div className='lg:hidden md:hidden block'>
                <div className="w-full px-4 py-4 overflow-x-hidden relative">
                    <div
                        ...
                        >
                        {
                            data?.map(({color, name, brand, salesPrice, badge, images,_id, otherImages }: TFrame, index: number) => <GlassCard color={color} images={images} otherImages={otherImages} badge={badge} salesPrice={salesPrice} name={name} brand={brand} key={index} _id={_id}/> )
                        }
