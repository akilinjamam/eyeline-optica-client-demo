...
import AddToCartForSingleAccessory from './AddToCartForSingleAccessory';

const SlideOptionsForAccessory = ({singleLens}: {singleLens:TAccessory}) => {
  ...
  const total = singleLens?.items?.map((item:TAccessoryItem) => item?.salesPrice)?.reduce((acc:number,sum:number) => acc + sum,0);

   const accessoryItems = {
      accessoryId:singleLens._id,
      type: "accessory",
      quantity: 1,
      unitPrice: total,
      subtotal: total,
    ...
    return (
        ...
        <AnimatePresence custom={direction} mode="popLayout">
          ...
          >
            ...
            {current.type === 'init' && (
              <div className="p-4">
                <AddToCartForSingleAccessory cartInfo={accessoryItems}/>
              </div>
            )}
...
export default SlideOptionsForAccessory;