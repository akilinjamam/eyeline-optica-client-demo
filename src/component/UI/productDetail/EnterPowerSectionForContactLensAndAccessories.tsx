...
const EnterPowerSectionForContactLensAndAccessories = ({singleLens, singleAccessory}:{singleLens:ILense, singleAccessory: any}) => {
  ...
  const createAxis = (start: number, end: number) => {
   ...
   const axisArr = createAxis(0, 181)
   
    const handleSubmit = async (e:any) => {
        e.preventDefault();
        const {name, pd, phoneNumber, email, address} = e.target;

        const totalPrice = singleLens.salesPrice + singleAccessory.total
            ...
            items: [
                {
                    ...
                    accessoryId: singleAccessory?.id,
                    pd: Number(pd.value),
...
export default EnterPowerSectionForContactLensAndAccessories;