...
const SidebarOld = ({ data }: { data: TFrame[] }) => {
  ...
  const frameSize = [...new Set(data.map((p: TFrame) => p.sizeCategory))]?.map(
    (size) => ({ title: size })
  );
  const material = [
    ...new Set(data.map((p: TFrame) => p.materialsCategory)),
  ]?.map((material) => ({ title: material }));
  const biology = [...new Set(data.map((p: TFrame) => p.biologyCategory))]?.map(
    (bio) => ({ title: bio })
  );
