        ...
        <div className='w-full mt-8'>
            <div  className="w-[90%] flex items-center gap-4 mx-auto">
                <hr className="flex-grow border-t border-gray-300" />
                    <p className="whitespace-nowrap text-[36px] font-medium">Best Selling</p>
                <hr className="flex-grow border-t border-gray-300" />
            </div>
            ...
            <div className='flex items-center justify-around w-[1000px] mx-auto gap-5'>
           
            <div ref={parentRef} className='w-[2600px] h-[300px] mx-auto flex items-center  gap-6 mt-2 overflow-x-hidden scroll-smooth  px-2 '>
                {
                    glassData?.slice(0,5).map(({colorCount, title, model, price, tag, image }: GlassCardProps, index: number) => <GlassCard colorCount={colorCount} image={image} tag={tag} price={price} title={title} model={model} key={index}/> )
                }
            </div>
